 ...
 * 数据库连接池
 * @author Ming
 * @description 改进的数据库连接池，参考http://blog.sina.com.cn/s/blog_55d57a460100iq71.html
 * @date 2015-03-17
 */
...
// 它中存放的对象为 PooledConnection 型
    ...
    private static void init() {
		try {
			// TODO 这种获取jdbc配置文件的路径有点问题，当路径中有空格的时候无法识别
			InputStream in = new FileInputStream(Thread.currentThread().getContextClassLoader().getResource(FILE_NAME).getPath());
    ...
    public ConnectionPool() {
    	if (!inited) {
	    	init();
	    	this.jdbcDriver = this.driverName;
	    	this.dbUrl = this.url;
	    	this.dbUsername = this.username;
	    	this.dbPassword = this.password;
	    	this.maxConnections = this.maxActive;
	    	this.initialConnections = this.maxIdle;
	    	ConsoleMessage.info("数据库连接池初始化成功");
	    	inited = true;
    	}
    	try {
        	this.createPool();
    ...
    public ConnectionPool(String filePath) {
    	if (!inited) {
	    	init(filePath);
	    	this.jdbcDriver = this.driverName;
	    	this.dbUrl = this.url;
...
//            System.out.println(" 数据库连接己创建 ......");
    ...
    public synchronized void refreshConnections() throws SQLException {
        ...
        while (enumerate.hasMoreElements()) {
            // 获得一个连接对象
            pConn = (PooledConnection) enumerate.nextElement();
            // 假如对象忙则等maxWait秒 ,maxWait秒后直接刷新
            if (pConn.isBusy()) {
                wait(maxWait); // 等maxWait秒
            }
            // 关闭此连接，用一个新的连接代替它。
